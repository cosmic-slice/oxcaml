...
let mancala_board ~(game_state : Game_state.t) ~set_game_state =
  ...
  let render_pit ~board_index ~is_goal ~player_class ~move_number =
    ...
    let maybe_clickable_attr =
      if is_game_over || is_goal || Option.is_none move_number
      then Vdom.Attr.empty
        ...
        Vdom.Attr.on_click (fun _ ->
          ...
          | Error _ ->
            raise_s [%message "Invalid move" (board_index : int) (player_class : string)]
  ...
  in
  Vdom.Node.div ~attrs:[ Vdom.Attr.class_ "game" ] [ board ]
;;
